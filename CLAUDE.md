...
- **Auth:** Microsoft OAuth (Entra-App von Eduskript mitgenutzt, Tenant `common`) + Magic-Link als Fallback (HMAC-Token, Versand über Brevo — gleiches Konto wie Eduskript). Bewusst ohne better-auth: dessen Pflicht-Tabellen (String-IDs) passen nicht zum Schema, zwei Flows direkt sind schlanker (`src/auth.ts`). Lesen/Suchen komplett offen; Melden und Upvoten nur mit Login (Kostenbremse: jede gemeldete URL erzeugt dauerhafte Crawl-/AI-Last). Später: Edulog als dritter Provider (Identitätsföderation Bildungsraum Schweiz, auch Sek II) — braucht Dienstleister-Registrierung bei Educa inkl. sponsernder Schule; Kandidaten via KSWE-/SVIA-Kontakte, sobald Atlas läuft.
- **Quellen-Modell:** Dedup über normalisierte URL (kein Duplikat bei erneuter Meldung; gleiche Domain schon vorhanden → Admin-Merge-Vorschlag). Man darf fremde öffentliche Inhalte melden; kein Besitzer-Konzept, Tote-Link-Mails gehen an die Melder:in. Pro Quelle: `quality_score` 0–100 von der AI (fünf Bänder untauglich/Rohmaterial/solide/lebendig/begeisternd; <20 = nicht aufgenommen; Ranking = Score + 5 × Netto-Votes) + Todescounter (fehlgeschlagene Crawl-Nächte in Folge, ab 3 ausgeblendet + Mail, Erfolg setzt zurück). Erfasst wird nur die URL (+ Connector-Typ, meist auto-erkannt; optional Fach) — Titel/Beschreibung extrahiert die AI beim ersten Crawl.
